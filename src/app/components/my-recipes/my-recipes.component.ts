...
import { infoUser, przepisDTO, wyswietlWszystko } from 'src/app/interfaces/interfejsy';
...
export class MyRecipesComponent {
  ...
  constructor(private api:ApiServiceService,protected modalService: ModalService){
    ...
    this.api.getPrzepisUser(user.idusers_game).subscribe({
      next:(response)=>{
        console.log(response)
        for(let i = 0; i<response.length; i++){
          console.log(response[i].nazwa, response[i].obraz, response[i].kategoria, response[i].przygotowanie,response[i].skladniki)
        }
